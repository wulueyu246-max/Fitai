...
| `source` | string | `source` | 统一来源标识：`mock` 或 `taobao`。 |
| `title` | string | `title` | 商品标题。 |
| `brand` | string | `brand` | 品牌。 |
| `category` | string | `category` | T恤、裤子、鞋、外套等分类。 |
| `price` | number | `price` | 当前价格，建议使用 decimal。 |
| `original_price` | number? | `original_price` | 可选原价，仅高于当前价时展示。 |
| `coupon_amount` | number | `coupon_amount` | 可选优惠券金额；Mock 固定为 0。 |
| `shop_name` | string | `shop_name` | 可选店铺名称。 |
...
| `purchase_url` | string | `purchase_url` | Flutter 唯一使用的购买地址，必须是 HTTPS。 |
| `recommendation_reason` | string | - | 为什么推荐该商品。 |
| `match_explanation` | string | - | 与当前穿搭关键词的匹配说明。 |
| `is_mock` | boolean | `is_mock` | 是否为测试商品；为真时禁止购买。 |
...
`MockProductProvider` 从 20 条内置测试商品中匹配，覆盖 T恤、裤子、鞋和外套。Mock 记录统一标记 `is_mock: true`，购买链接、佣金、PID 和优惠券均为空，不会模拟真实淘宝交易。配置完整的 `TAOBAO_APP_KEY`、`TAOBAO_APP_SECRET` 和 `TAOBAO_PID` 后，工厂才切换为 `TaobaoProductProvider`。原顶层 `products` 只保留 AI 的 `category/style/keyword` 查询条件，`recommendations.products` 返回 Provider 的统一商品记录。
...
2. 点击商品先记录详情点击；只有非 Mock 且存在有效 HTTPS `purchase_url` 时才开放购买按钮，不使用普通详情页伪装联盟购买链接。
...
```http
GET /products/recommend?category=外套&style=通勤&color=藏青&bodyType=肩窄&scene=工作&budget=1200&keyword=短款合体
```

参数均可选。响应为 `{ "products": [...] }`，每项均来自服务端商品目录。预算只有在用户请求中明确提供时才参与筛选。
